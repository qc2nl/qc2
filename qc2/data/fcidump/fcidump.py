...
class FCIDump:
    ...
    def to_file(self, fcidump: str | Path) -> None:
        ...
        Raises:
            ValueError: not all beta-spin related matrices are either None or not None.
            ValueError: if the dimensions of the provided integral matrices do not match.
        """
        ...
        if not all(h is None for h in [self.hij_b, self.hijkl_ba, self.hijkl_bb]) and not all(
            ...
            raise ValueError("Invalid beta variables.")
        ...
        with outpath.open("w", encoding="utf8") as outfile:
            ...
            if self.orbsym is None:
                ...
                if len(self.orbsym) != norb:
                    raise ValueError(f"Invalid number of orbitals {norb} {len(self.orbsym)}")
