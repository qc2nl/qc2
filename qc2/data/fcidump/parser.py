...
def _parse(fcidump: Path) -> FCIDump:
    ...
    Raises:
        ValueError: If the input file cannot be found, if a required field in the FCIDump
    ...
    try:
        ...
        raise ValueError(f"Input file '{fcidump}' cannot be read!") from ex
    ...
    if _norb is None:
        raise ValueError("The required NORB entry of the FCIDump format is missing!")
    norb = int(_norb.groups()[0])
    output["NORB"] = norb
    _nelec = re.search("NELEC" + pattern, metadata)
    if _nelec is None:
        raise ValueError("The required NELEC entry of the FCIDump format is missing!")
    ...
    s8_hijkl = np.zeros((norb, norb, norb, norb))
    hij_b = s4_hijkl_ba = s8_hijkl_bb = None
    if _uhf:
        hij_b = np.zeros((norb, norb))
        s4_hijkl_ba = np.zeros((norb, norb, norb, norb))
        s8_hijkl_bb = np.zeros((norb, norb, norb, norb))
